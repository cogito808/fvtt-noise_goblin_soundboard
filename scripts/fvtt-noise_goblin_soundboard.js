// scripts/[fvtt-noise_goblin_soundboardtt-soundboard.js)

class SoundBoardApplication extends Application {

static get defaultOptions() { const options = super.defaultOptions; options.title = 🔊${game.i18n.localize('SOUNDBOARD.app.title')}; options.id = 'soundboard-app'; options.template = 'modules/[options.template](https://opfvtt-noise_goblin_soundboard/templates/soundboard.html'; options.resizable = true; return options; }

async render(force = false, options = {}) { await super.render(force, options);
...
getData() { const sounds = []; let totalCount = 0;
...
const isExampleAudio = game.settings.get('fvtt-noise_goblin_soundboard', 'soundboardDirectory') === game.settings.get('fvtt-noise_goblin_soundboard', 'soundboardDirectory');
...
static async toggleExtendedOptions(element, identifyingPath, favTab) { if (!identifyingPath) return;
